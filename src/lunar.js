import { minYear, minMonth, minDay, maxYear, maxMonth } from "./config";
 ...
 * 
 */                
const lunarMonthMap = [0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,//1900-1909
...
export const getLeapMonth = (lYear) => {
  let data = lunarMonthMap[lYear - minYear];
  return data & 0xf;
}
 ...
 * @param {number} lYear 农历年
 * @returns {number} (-1、0、29、30)
 */
export const getLeapDays = (lYear) => {
  if (lYear > maxYear || lYear < minYear) {
    console.log(`不在规定年限范围${minYear}~${maxYear}内, 请重新输入`);
    return -1;
  }
  if (getLeapMonth(lYear)) {
    return ((lunarMonthMap)[lYear - minYear] & 0x10000) ? 30 : 29;
  }
  return 0;
}
...
export const getNonLeapMonthDays = (lYear, lMonth) => {
  if (lMonth > maxMonth || lMonth < minMonth) {
    console.log(`输入月份有误，月份应在${minMonth}~${maxMonth}内`)
    return -1;
  }
  return ((lunarMonthMap[lYear - minYear] & (0x10000 >> lMonth)) ? 30 : 29);
}
...
export const getlunarEachMonthDays = (lYear) => {
  if (lYear > maxYear || lYear < minYear) {
    console.log(`输入有误，年份应在${minYear}~${maxYear}内`)
    return [];
  }
  let lunarYear = lunarMonthMap[lYear - minYear];
  let monthDays = [];
  for(let i = 4; i < 16; i++) {
...
export const getLunarYearDays = (lYear) => {
  /**
   * 思路
   * 1. total 农历y年12个月份小月的总天数 29*12
   * 2. 通过循环判断出y年有几个大月 total+=1
   * 3. 计算出y年闰月的天数 total+天数
   * tip：
   *    1) 十六进制 0x8000 转成十进制为 32768 
   *    2) 十六进制 0x8  转为十进制为 8
   *    3) >> 右移 运算结果是除以2
  */
  let i, total = 348;
  // for(let i = 32768; i > 8; i /= 2)
  for (i = 0x8000; i > 0x8; i >>= 1) {
    // 进行与运算, 及转为二进制进行与运算
    total += (lunarMonthMap[lYear - minYear] & i) ? 1 : 0;
  }
 ...
 * @param {number} lYear 年
 * @param {number} lMonth 月
 * @param {boolean} isLeap 是否闰月
 * @returns {number} (-1、29、30)
*/
export const getLunarMonthDays = (lYear, lMonth, isLeap = false) => {
  if (lMonth > maxMonth || lMonth < minMonth) {
    console.log(`输入月数应在${minMonth}~${maxMonth}`)
    return -1;
  }
  let days = (lunarMonthMap[lYear - minYear] & (0x10000 >> lMonth)) ? 30 : 29;
 ...
 * @param {boolean} isSolarTime 是否公历的时间戳
 * @returns { ILunarInfo | null }
*/
export const getLunarByTimestamp = (timestamp, isSolarTime = false) => {
  // 计算出相对于1900-1-30的天数
  // Math.floor() 向下取整   Math.ceil() 向上取整
  // Tip: 如果是公历的时间戳，需减去时区差！！！ 
...
export const lunar2Solar = (lYear, lMonth, lDay, isLeapMonth) => {
  ...
  // 参数合法性校验
  if (lYear > maxYear || lYear < minYear) {
    console.log(`超出年份范围${minYear}~${maxYear}`);
    return -1;
  }
  if (lMonth > maxMonth || lMonth < minMonth) {
    console.log(`超出月份范围${minMonth}~${maxMonth}`);
    return -1;
  }
